...
# Set this to True if you need to debug clicks again.
DEBUG_INPUT = False
...
class BoardView(QGraphicsView):
    ...
    def __init__(self, theme, parent=None):
        ...
        # Important for lessons:
        # Some lesson positions can be marked by python-chess as game over
        # because of insufficient material, for example king + knight vs king.
        # Lessons may still require the user to move pieces.
        self.allow_game_over_positions = True
        ...
        self._fading_ghosts = []    # capture fade-out items
        ...
        self._renderers = {
            s: QSvgRenderer(SVG[s].encode("utf-8")) for s in SVG
        }
        ...
        # Important:
        # In some PySide6 setups QGraphicsView mousePressEvent is not reliable
        # because the real mouse events go to the viewport. Filtering viewport
        # events makes click handling much more stable.
    ...
    def pos_to_square(self, pos):
        ...
        # Tiny tolerance for edge clicks.
        if -0.5 < x < 0:
            x = 0.0
        if -0.5 < y < 0:
            y = 0.0
    ...
    def resize_to_fit(self, w, h):
        ...
        if self._fit_initialized and abs(s - self._S) < 0.5:
            return
        ...
        self.setFixedSize(int(board) + 2, int(board) + 2)
        self.setSceneRect(0.0, 0.0, board, board)
    ...
    def _kill_animations(self):
        ...
        # would never get its deleteLater() otherwise.
    ...
    def rebuild(self):
        ...
        for square, piece in self.board.piece_map().items():
            ...
            # Very important: pieces must not steal mouse events.
    ...
    def _animate_transition(self, prev, board, move):
        ...
        if prev.is_castling(move):
            kf, kt = chess.square_file(move.from_square), chess.square_file(move.to_square)
        ...
        if captured_square is not None:
            ...
            if cap_sym is not None:
                ...
                cap.setAcceptedMouseButtons(Qt.NoButton)
    ...
    def _refresh_overlays(self):
        ...
        if self._last_move is not None:
            ...
            for item, sq in (
                (self._last_from, self._last_move.from_square),
    ...
    def _human_to_move(self):
        ...
        # For lessons, do not block positions just because python-chess
        # considers them game over, for example insufficient material.
        if self.board.is_game_over() and not self.allow_game_over_positions:
            return False
    ...
    def _try_move(self, from_sq, to_sq):
        moves = [
            m for m in self._legal_for_selected if m.to_square == to_sq
        ]
        ...
        if needs_promo:
            promo = (
                self.choose_promotion(self.board.turn, to_sq)
                if self.choose_promotion
                else chess.QUEEN
            )
    ...
    def eventFilter(self, obj, event):
        """Make sure viewport mouse events are handled reliably."""
        if obj is self.viewport():
            ...
            if t == QEvent.MouseButtonPress:
                ...
                return self._dragging or self._press_piece is not None
    ...
    def mousePressEvent(self, e):
        ...
        if DEBUG_INPUT:
            print(
                ...
                "human_to_move=", self._human_to_move(),
        ...
        sq = self.pos_to_square(scene_pos)

        if DEBUG_INPUT:
            print("[board] scene_pos=", scene_pos, "square=", sq)
        ...
        if sq == self._selected:
            ...
            # Safety: if piece items got out of sync, rebuild once.
